...
$links = $pdo->query("
    ...
    FROM links l LEFT JOIN clicks c ON l.id = c.link_id
    GROUP BY l.id ORDER BY l.creato_il DESC LIMIT 10
")->fetchAll();
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        header nav a {
            ...
            letter-spacing: 0.5px; padding: 8px 16px; border-radius: 6px; transition: background 0.2s;
        }
        ...
        .stat-box {
            background: white; padding: 22px 18px; border-radius: 12px;
            box-shadow: 0 2px 8px rgba(0,0,0,0.06); border-left: 4px solid #d20a10; text-align: center;
        }
...
<body>
    ...
    new Chart(document.getElementById('chartLinea'), {
        ...
        data: {
            ...
            datasets: [{ label: 'Click', data: <?= json_encode($giorniData) ?>, borderColor: '#d20a10', backgroundColor: 'rgba(210,10,16,0.06)', borderWidth: 3, pointBackgroundColor: '#d20a10', pointRadius: 4, pointHoverRadius: 6, fill: true, tension: 0.4 }]
        },
    ...
    new Chart(document.getElementById('chartBarre'), {
        ...
        data: {
            labels: <?= json_encode($giorniLabel) ?>,
            datasets: [{ label: 'Click', data: <?= json_encode($giorniData) ?>, backgroundColor: 'rgba(210,10,16,0.12)', borderColor: '#d20a10', borderWidth: 2, borderRadius: 4, hoverBackgroundColor: 'rgba(210,10,16,0.3)' }]
        },
        options: { responsive: true, plugins: { legend: { display: false }, tooltip: { backgroundColor: '#000', titleColor: '#d20a10', bodyColor: '#fff', padding: 12, cornerRadius: 6 } }, scales: { x: { grid: { display: false } }, y: { beginAtZero: true, ticks: { stepSize: 1 }, grid: { color: '#f5f5f5' } } } }
    });
    <?php endif; ?>
