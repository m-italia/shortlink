...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validaURL($url)) {
        ...
        if (!$errore) {
            ...
            if (!$errore) {
                $stmt = $pdo->prepare("INSERT INTO links (codice, url_destinazione, titolo, qr_path, scade_il) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$codice, $url, $titolo ?: null, 'qrcodes/' . $codice . '.png', $scade_il ?: null]);
                $successo = $codice;
            }
        }
    }
...
<body>
    ...
    <div class="container">
        ...
        <div class="successo-box">
            ...
            <span class="short-url">http://localhost:8888/shortlink/<?= htmlspecialchars($successo) ?></span><br>
        ...
        <div class="layout">
            <div>
                ...
                <div class="card">
                    ...
                    <div class="sezione-label">Logo al centro <span class="hint" style="text-transform:none;">(opzionale — PNG, JPG o SVG)</span></div>
    ...
    function aggiornaAnteprima() {
        ...
        if (!url) { document.getElementById('anteprimaBox').innerHTML = '<div class="placeholder">Inserisci prima un URL valido</div>'; return; }
        ...
        .catch(() => { document.getElementById('anteprimaBox').innerHTML = '<div class="placeholder" style="color:#d20a10;">Errore di connessione</div>'; });
    }
